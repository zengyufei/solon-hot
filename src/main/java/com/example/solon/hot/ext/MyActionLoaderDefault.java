 ...
 * @since 1.0
 */
public class MyActionLoaderDefault extends HandlerAide implements ActionLoader {
    ...
    protected List<Routing<Handler>> routings;

    public MyActionLoaderDefault(BeanWrap wrap) {
        ...
        routings = Solon.app().router().getAll(Endpoint.main).stream()
                .filter(e->e.target() instanceof ActionDefault)
                .filter(e -> ((ActionDefault) e.target()).controller().clz() == clz)
                .collect(Collectors.toList());
    }

    public MyActionLoaderDefault(BeanWrap wrap, String mapping, boolean remoting, Render render, boolean allowMapping) {
    ...
    protected void loadActions(HandlerSlots slots, boolean all) {
        ...
        Map<Method, MyAction> map = new HashMap<>();
        //只支持 public 函数为 Action
        for (Method method : findMethods(bw.clz())) {
            loadActionItem(map, slots, all, method, b_limitMethodSet, b_addinMethodSet);
        }

        for (Map.Entry<Method, MyAction> entry : map.entrySet()) {
            final MyAction myAction = entry.getValue();

            slots.add(myAction.getNewPath(), myAction.getM1(), myAction.getAction());
        }

        if (!routings.isEmpty()) {
            for (Routing<Handler> routing : routings) {
                Solon.app().router().remove(routing.path());
            }
        }
        System.out.println(Solon.app().router().getAll(Endpoint.main));
    }

    /**
     * 加载 Action item 处理
     */
    protected void loadActionItem(Map<Method, MyAction> map, HandlerSlots slots, boolean all, Method method, Set<MethodType> b_limitMethodSet, Set<MethodType> b_addinMethodSet) {
        ...
        if (m_map != null || all) {
            ...
            for (MethodType m1 : m_limitMethodSet) {
                ...
                for (Routing<Handler> routing : routings) {
                    if (routing.matches(routing.method(), newPath)) {
                        isFind = true;
                ...
                final MyAction myAction = new MyAction(method, newPath, m1, action);
                map.put(method, myAction);
